...
Input files:
  ...
  - data/raw/player_dim.csv

Output:
  - data/processed/hockey_kg.ttl
...
PLAYER_STATS_CSV = RAW_DATA_DIR / "player_stats.csv"
PLAYER_DIM_CSV = RAW_DATA_DIR / "player_dim.csv"
OUTPUT_TTL = PROCESSED_DATA_DIR / "hockey_kg.ttl"
...
print("Loading CSVs...")
try:
    stats = pd.read_csv(PLAYER_STATS_CSV, encoding="latin-1")
    dim   = pd.read_csv(PLAYER_DIM_CSV,   encoding="latin-1")
except FileNotFoundError as e:
    print(f"ERROR: {e}")
    print(f"Expected files at: {RAW_DATA_DIR}")
    sys.exit(1)
...
print(f"\nNext step: load {OUTPUT_TTL} into GraphDB or Apache Jena Fuseki!")
